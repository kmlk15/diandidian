...
class BagServiceTest extends Specification {
  ...
  " 地点分配日期 测试"  in new WithApplication {
   ...
   " 索引 Plan 测试"  in new WithApplication {

      val mongoDB = MongoConnection()("mytestxx")
      mongoDB("bags").drop()
      mongoDB("locationPlanIndex").drop()
      
      val location1 = LocationForm(id = Some("1"), name = "l1", enName = "l1en")
      val location2 = LocationForm(id = Some("2"), name = "l2", enName = "l2en")
      val location3 = LocationForm(id = Some("3"), name = "l3", enName = "l3en")
      val location4 = LocationForm(id = Some("4"), name = "l4", enName = "l4en")
      val location5 = LocationForm(id = Some("5"), name = "l5", enName = "l5en")
      ...
      
      {
        ...
        val jsVal = service.updatePlanVisible(bagId, statusName, planName, "public")
        ( jsVal \"success" ).as[Boolean]  === true
        
        val indexList = service.getLocationPlanIndex(location1.id.get)
        indexList.size === 1 
        indexList.head.bagId ===  bagId
        indexList.head.planId === plan.id
      ...
      {
        //设置为 私有
        val jsVal = service.updatePlanVisible(bagId, statusName, planName, "private")
        ( jsVal \"success" ).as[Boolean]  === true
      ...
      
       {
        ...
        val indexList = service.getLocationPlanIndex(location2.id.get)
        indexList.size === 1 
        indexList.head.bagId ===  bagId
        indexList.head.planId === plan.id
        ...
        service.addLocation(bagId, typ, usertype, statusName, planName, location4)
        service.addLocation(bagId, typ, usertype, statusName, planName, location5)
  
        val indexList1 = service.getLocationPlanIndex(location1.id.get)
        indexList1.size === 1 
   
        val indexList4 = service.getLocationPlanIndex(location4.id.get)
        indexList4.size === 1 

        
        service.removeLocation(bagId, statusName, planName, location2)
         val indexList2 = service.getLocationPlanIndex(location2.id.get)
        indexList2.size === 0                
